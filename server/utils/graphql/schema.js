...
export async function buildEntuContext (event) {
  ...
  const tokenStr = (getHeader(event, 'authorization') || '').replace('Bearer ', '').trim()
...
async function computeSchemaHash (entu) {
  ...
  if (!result.length) return '0:null'
...
async function buildSchema (entu) {
  ...
  for (const doc of propDocs) {
    ...
    if (!propsByTypeId[parentId]) propsByTypeId[parentId] = []
...
function getInputType (propDef) {
  if (propDef.multilingual) return '[MultilingualStringInput]'
