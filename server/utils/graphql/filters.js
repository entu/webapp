...
export function toGqlFieldName (name) {
  const segments = name.split(/[\s_-]+/).map((s) => s.replace(/[^A-Za-z0-9]/g, '')).filter(Boolean)
  ...
  const result = segments
    ...
    .join('')
  return /^[0-9]/.test(result) ? `f${result}` : (result || 'unknown')
}
...
export function toGqlTypeName (name) {
  ...
  return /^[0-9]/.test(result) ? `T${result}` : (result || 'Unknown')
}
