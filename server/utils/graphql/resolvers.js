  ...
  for (const entityType of entityTypes) {
    ...
    Query[fieldName] = async (_, args, { entu }) => {
      ...
      if (args.filter) {
        ...
        if (args.filter._search) {
          const terms = args.filter._search.toLowerCase().split(' ')
            .filter(Boolean)
    ...
    if (!hasWritableProps) {
      ...
      Mutation[`${fieldName}Delete`] = async (_, { id }, { entu }) => {
        if (!entu.user) throw new Error('Authentication required')
        ...
        if (!owners.includes(entu.userStr)) throw new Error('User not in _owner property')
    ...
    Mutation[`${fieldName}Update`] = async (_, { id, input }, { entu }) => {
      ...
      if (newProps.length === 0) throw new Error('At least one non-null property value is required for update')
      ...
      for (const p of existingProps) {
        if (!oldIdsByType[p.type]) oldIdsByType[p.type] = []
